...
if [ "$ENABLE_PREEMPTIVE_AUTO_PROVISIONING" = "true" ]; then
  parallel-ssh -t 0 --host $HOST "cd Block && export PYTHONPATH=. && nohup python route_balance/global_scheduler/api_server.py --config_path $CONFIG_PATH --metrics_type $METRICS_TYPE --num_query_predictor $M --num_required_predictor $N --workers $NUM_WORKERS --num_predictor_ports $NUM_PREDICTORS --profiling_sampling_rate $PROFILING_SAMPLE_RATE --predictor_timeout $PREDICTOR_TIMEOUT --backend_timeout $BACKEND_TIMEOUT --initial_available_instance $AVAILABLE_INSTANCE --max_slo_in_seconds $MAX_SLO --use_preemptive_provisioning > experiment_output/logs/global_scheduler.log 2>&1 &"
else
