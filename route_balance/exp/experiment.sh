...
PREDICTOR_CONFIG_PATH="route_balance/config/${MODEL_TYPE}_config.json"
...
if [ "$RESTART_VLLM" = "true" ]; then
  ...
  sh route_balance/exp/reset.sh
  if [ "$UPDATE_BLOCK_CODE" = "true" ]; then
    parallel-ssh -t 0 -h route_balance/config/hosts "cd Block && git checkout $BRANCH_NAME"
    parallel-ssh -t 0 -h route_balance/config/hosts "cd Block && git add -u . && git stash && git reset --hard HEAD~1 && git pull"
  fi
  sleep 60
  nohup sh route_balance/exp/run_exp_vllm.sh $BATCH_CAP $MODEL $UPDATE_VLLM_CODE $VLLM_VERSION $MAX_MODEL_LENGTH $ENABLE_CHUNKED_PREFILL $BACKEND_WORKERS $MAX_NUM_BATCHED_TOKEN > /dev/null 2>&1 &
  sleep 60
  script_base="route_balance/exp/run_exp_predictor"
        ...
        for metric_type in $METRIC_TYPES; do
          ...
          for n in $N; do
              for use_estimation_len in $USE_ESTIMATION_LEN; do
                  ...
                  nohup sh route_balance/exp/run_exp_global_scheduler.sh $TARGET_HOST $n $n $metric_type $HOST_CONFIG_PATH $GLOBAL_SCHEDULER_WORKERS $PREDICTOR_WORKERS $PROFILING_SAMPLE_RATE $TIMEOUT_IN_SECONDS $PREDICTOR_TIMEOUT_IN_SECONDS $AVAILABLE_INSTANCE $MAX_SLO $ENABLE_PREEMPTIVE_AUTO_PROVISIONING > /dev/null 2>&1 &
