...
parallel-ssh -t 0 -h route_balance/config/p100_smoke/hosts "git clone ${BLOCK_GITHUB_LINK} && cd RouteBalance && git checkout route_balance  && pip install -r requirements.txt"
...
parallel-ssh -t 0 -h route_balance/config/p100_smoke/ampere_hosts "git clone ${BLOCK_GITHUB_LINK} && cd RouteBalance && git checkout route_balance  && pip install -r requirements.txt"
...
parallel-ssh -t 0 -h route_balance/config/p100_smoke/pascal_hosts "cd vllm && sudo PATH=\"/users/anon/.local/bin:\$PATH\" CUDACXX=/usr/local/cuda-12.8/bin/nvcc TORCH_CUDA_ARCH_LIST=6.0 MAX_JOBS=4 CMAKE_BUILD_PARALLEL_LEVEL=4 pip install --editable ."
