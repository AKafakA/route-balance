#!/bin/bash
# LPT Sort Key Ablation: max / min / mean
set -euo pipefail
RESULT_DIR=${1:-"experiment_output/ablation/lpt_sort"}
NUM_REQ=${2:-2000}
...
echo "LPT Sort Ablation — $(date)"
for KEY in max min mean; do
    cfg "{\"slo_defaults\":{\"lpt_sort_key\":\"$KEY\"}}"
    echo "  key=$KEY"
    for QPS in $QPS_LEVELS; do run "lpt_${KEY}_qps${QPS}" --request-rate $QPS; done
done
cfg '{"slo_defaults":{"lpt_sort_key":"max"}}'
echo "LPT Sort COMPLETE"
