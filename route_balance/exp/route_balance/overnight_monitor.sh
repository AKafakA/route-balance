...
LOG="/home/anon/Code/llm/RouteBalance/route_balance_paper/claude/OVERNIGHT_MONITOR.md"
...
check_csd3() {
    ...
    QUEUE=$(ssh -o ConnectTimeout=10 -o BatchMode=yes csd3 'squeue -u anon 2>/dev/null' 2>/dev/null)
    if [ -z "$QUEUE" ]; then
    ...
    COMPLETED=$(ssh -o ConnectTimeout=10 csd3 'sacct -u anon --starttime=2026-03-29 --format=JobID,JobName%15,State,Elapsed --noheader 2>/dev/null | grep -E "COMPLETED|FAILED" | grep -v ".batch\|.extern"' 2>/dev/null)
    if [ -n "$COMPLETED" ]; then
        echo ""
