...
LOGFILE="/home/wd312/Code/llm/Block/route_balance_paper/plans/gpu_vm_monitor.log"
REMOTE="wd312@gxp-l40s-2.cl.cam.ac.uk"
REPORT="/home/wd312/Code/llm/Block/route_balance_paper/plans/experiment_report_march23_25.md"
...
STATUS=$(ssh -o ConnectTimeout=10 "$REMOTE" 'cd ~/Code/llm/Block 2>/dev/null || exit 1

echo "DISK_FREE=$(df -BG /home/wd312 --output=avail | tail -1 | tr -d " G")"
...
if [ -n "$CKPT_COUNT" ] && [ "$CKPT_COUNT" -gt 0 ]; then
    ...
    ssh -o ConnectTimeout=10 "$REMOTE" 'cd ~/Code/llm/Block && for model_dir in models/route_balance/study/*/  models/route_balance/long_study/*/ models/route_balance/early_study/*/; do
    ...
    find ~/Code/llm/Block/models/route_balance -name "optimizer.pt" -not -path "*/checkpoint-*/*" -delete 2>/dev/null
    find ~/Code/llm/Block/models/route_balance -name "rng_state*.pth" -not -path "*/checkpoint-*/*" -delete 2>/dev/null' 2>/dev/null
...
if [ "$RUNNING" = "none" ]; then
    ...
    RERUN_DONE=$(ssh -o ConnectTimeout=10 "$REMOTE" 'ls ~/Code/llm/Block/models/route_balance/study/qwen05b_fused_judge/adapter_config.json 2>/dev/null && echo yes || echo no' 2>&1)
    LONG_DONE=$(ssh -o ConnectTimeout=10 "$REMOTE" 'ls ~/Code/llm/Block/models/route_balance/long_study/modernbert_fused_length_mse/training_results.json 2>/dev/null && echo yes || echo no' 2>&1)

    if [ "$RERUN_DONE" = "no" ]; then
        echo "$(timestamp) Launching failed experiment re-runs..." >> "$LOGFILE"
        ssh -o ConnectTimeout=10 "$REMOTE" 'cd ~/Code/llm/Block && nohup bash route_balance/exp/route_balance/run_gpu_vm_rerun_failed.sh > /tmp/rerun_failed.log 2>&1 &' 2>/dev/null
        ...
        if [ "$LONG_LAUNCHED" = "no" ]; then
            ...
            ssh -o ConnectTimeout=10 "$REMOTE" 'cd ~/Code/llm/Block && touch /tmp/long_training_launched && nohup bash route_balance/exp/route_balance/run_gpu_vm_long_training.sh > /tmp/long_training.log 2>&1 &' 2>/dev/null
        ...
        JUDGE_DONE=$(ssh -o ConnectTimeout=10 "$REMOTE" 'ls ~/Code/llm/Block/models/route_balance/study/modernbert_fused_judge_class/training_results.json 2>/dev/null && echo yes || echo no' 2>&1)
        if [ "$JUDGE_DONE" = "no" ]; then
            echo "$(timestamp) Long training done. Launching judge_class retries..." >> "$LOGFILE"
            ssh -o ConnectTimeout=10 "$REMOTE" 'cd ~/Code/llm/Block && nohup bash route_balance/exp/route_balance/run_gpu_vm_judge_retry.sh > /tmp/judge_retry.log 2>&1 &' 2>/dev/null
            echo "$(timestamp) Judge retries launched" >> "$LOGFILE"
        else
            # Check if final eval done
            EVAL_DONE=$(ssh -o ConnectTimeout=10 "$REMOTE" 'ls ~/Code/llm/Block/models/route_balance/evaluation/eval_length.json 2>/dev/null && echo yes || echo no' 2>&1)
            if [ "$EVAL_DONE" = "no" ]; then
                echo "$(timestamp) All training done. Launching final evaluation..." >> "$LOGFILE"
                ssh -o ConnectTimeout=10 "$REMOTE" 'cd ~/Code/llm/Block && nohup bash route_balance/exp/route_balance/run_gpu_vm_final_eval.sh > /tmp/final_eval.log 2>&1 &' 2>/dev/null
                echo "$(timestamp) Final evaluation launched" >> "$LOGFILE"
            else
                echo "$(timestamp) ALL COMPLETE (training + evaluation)" >> "$LOGFILE"

                # Collect all results
                ssh -o ConnectTimeout=10 "$REMOTE" 'cd ~/Code/llm/Block && find models/route_balance/evaluation -name "*.json" 2>/dev/null | sort | while read f; do echo "=== $f ==="; cat "$f" | python3 -c "import sys,json; d=json.load(sys.stdin); print(json.dumps(d,indent=2)[:1000])" 2>/dev/null; done' >> "$LOGFILE" 2>&1
