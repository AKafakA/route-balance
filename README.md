...
**Model-estimator dataset** — released on the Hugging Face Hub: [`asdwb/route_balance_model_estimator`](https://huggingface.co/datasets/asdwb/route_balance_model_estimator). 18,553 scored prompts (from 18,608 raw; recommended split `train.jsonl` 14,919 / `test.jsonl` 3,634) broadcast across the four Qwen2.5 candidates, drawn from seven public datasets (RewardBench, CodeUltraFeedback, BeaverTails, MixInstruct, LMSYS-Chat-1M, GSM8K, SQuAD). Each entry carries a dataset-specific `reference_text` and six per-(prompt, model) quality signals: embedding similarity, reference similarity, blind LLM-judge, **reference-grounded LLM-judge** (`deepeval-llama3.1-8b-it_reference`), output length, and a 16-bucket length label. The full prompt → response → labels generation pipeline lives under `route_balance/predictor/route_balance/offline_training/`.
